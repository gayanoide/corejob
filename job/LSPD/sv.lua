...
AddEventHandler('finalpolice:spawned', function()
	...
	if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'lspd' and xPlayer ~= nil and xPlayer.job2 ~= nil and xPlayer.job2.name == 'lspd' then
		Citizen.Wait(2500)
...
AddEventHandler('TireEntenduServeur', function(gx, gy, gz)
	...
	for i = 1, #xPlayers, 1 do
		...
		if thePlayer.job.name == 'lspd' or thePlayer.job2.name == 'lspd' then
...
AddEventHandler('Huid_police:DemandeRenfortPolice', function(coords, titre, raison)
    local xPlayers = ESX.GetExtendedPlayers('job', 'lspd')
...
AddEventHandler("genius:sendcall", function()
	...
	for i = 1, #xPlayers, 1 do
		local thePlayer = ESX.GetPlayerFromId(xPlayers[i])
		if thePlayer.job.name == 'lspd' or thePlayer.job2.name == 'lspd' then
			TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], "Central LSPD", nil, "Un Citoyen demande un agent de police au commissariat", "CHAR_LSPD", 8)
		end
	end
end)
...
--AddEventHandler('hlspd:Ouvert', function(source)
...
--AddEventHandler('hlspd:Fermer', function(source)
